...
rlist_node_t* rlist_find(rlist_t *self, void *val) {
  rlist_iterator_t *it = rlist_iterator_new(self, RLIST_HEAD);
  rlist_node_t *node;

  while ((node = rlist_iterator_next(it))) {
    if (self->match) {
      if (self->match(val, node->val)) {
        rlist_iterator_destroy(self, it);
        return node;
      }
    } else {
      if (val == node->val) {
...
rlist_node_t* rlist_at(rlist_t *self, int index) {
  rlist_direction_t direction = RLIST_HEAD;

  if (index < 0) {
    direction = RLIST_TAIL;
    index = ~index;
  }

  if ((unsigned)index < self->len) {
    rlist_iterator_t *it = rlist_iterator_new(self, direction);
    rlist_node_t *node = rlist_iterator_next(it);
    while (index--) node = rlist_iterator_next(it);
    rlist_iterator_destroy(self, it);
    return node;
  }
...
rlist_node_t* rlist_iterator_next(rlist_iterator_t *self) {
    ...
    if (curr) {
        self->next = self->direction == RLIST_HEAD
            ? curr->next
            : curr->prev;
    }
    return curr;
