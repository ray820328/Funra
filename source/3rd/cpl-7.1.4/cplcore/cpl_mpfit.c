...
/*
 * The function declarations of this module have been modified by adding
 * the prefix '_cpl_' to static functions, and 'cpl_' to public functions
 * to protect the namespace.
 */

/* Forward declarations of functions in this module */

static int _cpl_mp_fdjac2(mp_func funct,
...
static void _cpl_mp_qrfac(int m, int n, double *a, int lda,
	      int pivot, int *ipvt, int lipvt,
	      double *rdiag, double *acnorm, double *wa);
static void _cpl_mp_qrsolv(int n, double *r, int ldr, int *ipvt, double *diag,
	       double *qtb, double *x, double *sdiag, double *wa);
static void _cpl_mp_lmpar(int n, double *r, int ldr, int *ipvt, int *ifree, double *diag,
	      double *qtb, double delta, double *par, double *x,
	      double *sdiag, double *wa1, double *wa2);
static double _cpl_mp_enorm(int n, double *x);
static double _cpl_mp_dmax1(double a, double b);
static double _cpl_mp_dmin1(double a, double b);
static int _cpl_mp_min0(int a, int b);
static int _cpl_mp_covar(int n, double *r, int ldr, int *ipvt, double tol, double *wa);
 ...
 *
 *  Note: The original macro was modified to use a CPL memory allocator,
 *        and the call to malloc has been replaced with a call to calloc,
 *        which makes the initialization loop unnecessary.
 */
#define mp_malloc(dest,type,size) \
  dest = (type *) cpl_calloc( size, sizeof(type) ); \
  if (dest == 0) { \
    info = MP_ERR_MEMORY; \
    goto CLEANUP; \
  } 
...
int cpl_mpfit(mp_func funct, int m, int npar, double *xall, mp_par *pars,
              mp_config *config, void *private_data, mp_result *result)
{
 ...
 OUTER_LOOP:
  ...
  iflag = _cpl_mp_fdjac2(funct, m, nfree, ifree, npar, xnew, fvec, fjac,
  ...
  _cpl_mp_qrfac(m,nfree,fjac,ldfjac,1,ipvt,nfree,wa1,wa2,wa3);
  ...
  if (fnorm != zero) {
	...
	gnorm = _cpl_mp_dmax1(gnorm,fabs(sum/wa2[l]));
      }
      jj += m;
    }
  }
  ...
  if (conf.douserscale == 0) {
    for (j=0; j<nfree; j++ ) {
      diag[ifree[j]] = _cpl_mp_dmax1(diag[ifree[j]],wa2[j]);
    }
  }
 ...
 L200:
   ...
   */
  _cpl_mp_lmpar(nfree,fjac,ldfjac,ipvt,ifree,diag,qtf,delta,&par,
                wa1,wa2,wa3,wa4);
  /*
  ...
  if (qanylim == 0) {
	...
	alpha = _cpl_mp_dmin1(alpha, (llim[j]-x[j])/wa1[j]);
      }
      if (dwa1 && qulim[j] && ((x[j] + wa1[j]) > ulim[j])) {
	alpha = _cpl_mp_dmin1(alpha, (ulim[j]-x[j])/wa1[j]);
      }
    }
  ...
  pnorm = _cpl_mp_enorm(nfree,wa3);
  
  /*
   *	    on the first iteration, adjust the initial step bound.
   */
  if (iter == 1) {
    delta = _cpl_mp_dmin1(delta,pnorm);
  }
  ...
  temp1 = _cpl_mp_enorm(nfree,wa3)*alpha/fnorm;
  ...
  if (ratio <= p25) {
    ...
    delta = temp*_cpl_mp_dmin1(delta,pnorm/p1);
    par = par/temp;
  } else {
 ...
 L300:
  ...
  if (result && (result->covar || result->xerror)) {
    _cpl_mp_covar(nfree, fjac, ldfjac, ipvt, conf.covtol, wa2);
  ...
  if (result) {
    ...
    result->bestnorm = _cpl_mp_dmax1(fnorm,fnorm1);
 ...
 CLEANUP:
  if (fvec) cpl_free(fvec);
  if (qtf)  cpl_free(qtf);
  if (x)    cpl_free(x);
  if (xnew) cpl_free(xnew);
  if (fjac) cpl_free(fjac);
  if (diag) cpl_free(diag);
  if (wa1)  cpl_free(wa1);
  if (wa2)  cpl_free(wa2);
  if (wa3)  cpl_free(wa3);
  if (wa4)  cpl_free(wa4);
  if (ipvt) cpl_free(ipvt);
  if (pfixed) cpl_free(pfixed);
  if (step) cpl_free(step);
  if (dstep) cpl_free(dstep);
  if (mpside) cpl_free(mpside);
  if (ddebug) cpl_free(ddebug);
  if (ddrtol) cpl_free(ddrtol);
  if (ddatol) cpl_free(ddatol);
  if (ifree) cpl_free(ifree);
  if (qllim) cpl_free(qllim);
  if (qulim) cpl_free(qulim);
  if (llim)  cpl_free(llim);
  if (ulim)  cpl_free(ulim);
...
static 
int _cpl_mp_fdjac2(mp_func funct,
                   int m, int n, int *ifree, int npar, double *x, double *fvec,
                   double *fjac, int ldfjac, double epsfcn,
                   double *wa, void *priv, int *nfev,
                   double *step, double *dstep, int *dside,
...
*/
  ...
  temp = _cpl_mp_dmax1(epsfcn,MP_MACHEP0);
  eps = sqrt(temp);
  ij = 0;
  CPL_UNUSED(ldfjac);
...
static 
void _cpl_mp_qrfac(int m, int n, double *a, int lda,
                   int pivot, int *ipvt, int lipvt,
                   double *rdiag, double *acnorm, double *wa)
{
/*
	...
	for (k=jp1; k<n; k++)
	  {
		...
		temp = _cpl_mp_dmax1( zero, one-temp*temp );
		rdiag[k] *= sqrt(temp);
		temp = rdiag[k]/wa[k];
		if ((p05*temp*temp) <= MP_MACHEP0)
		  {
		    rdiag[k] = _cpl_mp_enorm(m-j-1,&a[jp1+m*k]);
		    wa[k] = rdiag[k];
		  }
	      }
	  }
...
static 
void _cpl_mp_qrsolv(int n, double *r, int ldr, int *ipvt, double *diag,
	       double *qtb, double *x, double *sdiag, double *wa)
{
/*
*     **********
...
static 
void _cpl_mp_lmpar(int n, double *r, int ldr, int *ipvt, int *ifree, double *diag,
                   double *qtb, double delta, double *par, double *x,
                   double *sdiag, double *wa1, double *wa2)
{
  /*     **********
  ...
  gnorm = _cpl_mp_enorm(n,wa1);
  paru = gnorm/delta;
  if (paru == zero)
    paru = MP_DWARF/_cpl_mp_dmin1(delta,p1);
  /*
   *     if the input par lies outside of the interval (parl,paru),
   *     set par to the closer endpoint.
   */
  *par = _cpl_mp_dmax1( *par,parl);
  *par = _cpl_mp_dmin1( *par,paru);
  if (*par == zero)
    *par = gnorm/dxnorm;
 ...
 L150:
   ...
   */
  if (*par == zero)
    *par = _cpl_mp_dmax1(MP_DWARF,p001*paru);
  temp = sqrt( *par );
  for (j=0; j<n; j++)
    wa1[j] = temp*diag[ifree[j]];
  _cpl_mp_qrsolv(n,r,ldr,ipvt,wa1,qtb,x,sdiag,wa2);
  for (j=0; j<n; j++)
    wa2[j] = diag[ifree[j]]*x[j];
  dxnorm = _cpl_mp_enorm(n,wa2);
  temp = fp;
   ...
   */
  if (fp > zero)
    parl = _cpl_mp_dmax1(parl, *par);
  if (fp < zero)
    paru = _cpl_mp_dmin1(paru, *par);
  /*
   *	 compute an improved estimate for par.
   */
  *par = _cpl_mp_dmax1(parl, *par + parc);
  /*
   *	 end of an iteration.
   */
  goto L150;
...
static 
double _cpl_mp_enorm(int n, double *x)
{
  /*
   *     **********
   *
...
static 
double _cpl_mp_dmax1(double a, double b)
{
  if (a >= b)
    return(a);
  else
...
static 
double _cpl_mp_dmin1(double a, double b)
{
  if (a <= b)
    return(a);
  else
...
static 
int _cpl_mp_covar(int n, double *r, int ldr, int *ipvt, double tol, double *wa)
{
  int i, ii, j, jj, k, l;
