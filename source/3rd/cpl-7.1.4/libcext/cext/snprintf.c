...
#include "snprintf.h"
    ...
    !defined(HAVE_ASPRINTF) || !defined(HAVE_VASPRINTF)
...
main(void)
{
    const char *float_fmt[] = {
        ...
        "%5.8e",
        "%+4.9e",
#if !defined(OS_LINUX)  /* glibc sometimes gets these wrong. */
        "%+#010.0e",
        "%#10.1e",
        "%10.5e",
        "% 10.5e",
    ...
    double float_val[] = {
        ...
        -1.0,
        -0.1,
#if !defined(OS_BSD)    /* BSD sometimes gets these wrong. */
#ifdef INFINITY
        INFINITY,
        -INFINITY,
...
#if !defined(OS_SYSV)   /* SysV uses a different format than we do. */
