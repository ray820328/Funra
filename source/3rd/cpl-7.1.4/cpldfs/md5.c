...
inline static void byteReverse(unsigned char * restrict buf, unsigned longs)
    CPL_ATTR_NONNULL;
#else
...
/* Each function has been redeclared so:
   1) each array argument is now a restricted pointer
   2) the nonnull attribute is used when available
   3) it is static (the whole md5.c code is included in the single
      source code file where it is used)

   And finally in MD5Final():
   4) the assignments causing a strict-aliasing warning have been rewritten
   5) The widely noticed sizeof bug in the "In case it's sensitive" memset()
      has been fixed
*/
 ...
 */
static void MD5Init(struct MD5Context * restrict ctx)
{
 ...
 * of bytes.
 */
static void MD5Update(struct MD5Context * restrict ctx,
                      unsigned char const * restrict buf, unsigned len)
{
    register word32 t;
...
static void MD5Final(unsigned char * restrict digest,
                     struct MD5Context * restrict ctx)
{
    unsigned int count;
    unsigned char *p;
    ...
    *p++ = 0x80;
    ...
    /* Change the original assigments to avoid the gcc-4.8.1 warning:
       dereferencing type-punned pointer will break strict-aliasing rules
       [-Werror=strict-aliasing]
    */
    (void)memcpy(ctx->in + 56, ctx->bits, 8);
